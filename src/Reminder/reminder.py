...
def list_events(bot):
    ...
    bot.send_message("1501345393", message)
        ...
        if (
            ...
            task_markup.add(
                InlineKeyboardButton(
                    f"{message} - {task.name} {task.task_date} {task.description}",
            ...
            task_markup.row(
                ...
                InlineKeyboardButton(
                    "Completada", callback_data=f"completed_{task.name}"
                ),
            )
    bot.send_message("1501345393", message, reply_markup=task_markup)


def change_item_reminder(bot, call) -> None:
    markup = InlineKeyboardMarkup()
    hours = [
        InlineKeyboardButton(str(i), callback_data=f"hour_{i}") for i in range(0, 24)
    ]
...
def ask_new_minute(bot, call):
    ...
    markup = InlineKeyboardMarkup()
    minutes = [
        InlineKeyboardButton(str(i), callback_data=f"minute_{i}") for i in range(0, 60)
    ]
